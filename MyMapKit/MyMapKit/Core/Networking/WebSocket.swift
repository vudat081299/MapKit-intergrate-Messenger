...
//  MyMapKit
//
//  Created by Vũ Quý Đạt  on 08/12/2019.
//

/*
 Project quản lý datagram & payload thông qua Core iOS.
 - Vũ Quý Đạt -
 chú thích, chức năng: extract file build của project SSVS:
    - StreamFile
    - realtime DB
    - Từ File Controller tương tác với cả ws và protocol MQTT: ứng dụng message app, IoT,..
    - StreamChunkedData
...
public struct WebSocketEventsAlpha {
...
public enum WebSocketErrorController : Error, CustomStringConvertible {
    case memory
    case needMoreInput
    case invalidHeader
    case invalidAddress
    ...
    func append(_ byte : UInt8) throws {
        if count == 0 {
            ...
            if byte == 0xC0 || byte == 0xC1 {
                throw WebSocketErrorController.payloadError("invalid codepoint: invalid byte")
            }
            if byte >> 5 & 0x7 == 0x6 {
                count = 2
                ...
                throw WebSocketErrorController.payloadError("invalid codepoint: frames")
            }
            procd = 1
        ...
        if byte >> 6 & 0x3 != 0x2 {
            throw WebSocketErrorController.payloadError("invalid codepoint: signature")
        }
        codepoint += UInt32(byte & 0x3F) << ((count-procd-1) * 6)
        if codepoint > 0x10FFFF || (codepoint >= 0xD800 && codepoint <= 0xDFFF) {
            throw WebSocketErrorController.payloadError("invalid codepoint: out of bounds")
        }
        procd += 1
        if procd == count {
            if codepoint <= 0x7FF && count > 2 {
                throw WebSocketErrorController.payloadError("invalid codepoint: overlong")
            }
...
private class FrameWS {
    var inflate = false
    var code = OpCode.continue
    var utf8 = UTF8()
    var payload = PayloadAlpha()
    var statusCode = UInt16(0)
    var finished = true
    static func makeClose(_ statusCode: UInt16, reason: String) -> FrameWS {
        let f = FrameWS()
        f.code = .close
...
private func zerror(_ res : CInt) -> Error? {
    ...
    return WebSocketErrorController.payloadError("zlib: \(err): \(res)")
}
        ...
        for i in 0 ..< 2{
            ...
            while true {
                ...
                if bufsiz == 0 {
                    ...
                    if nbuf == nil {
                        throw WebSocketErrorController.payloadError("memory")
                    }
                    buffer = nbuf
...
private class InnerWebSocket: Hashable {
    ...
    var _event = WebSocketEventsAlpha()
    ...
    var event : WebSocketEventsAlpha {
    ...
    func step(){
        ...
        do {
        ...
        } catch WebSocketErrorController.needMoreInput {
            more = true
        } catch {
            if finalError != nil {
                return
            ...
            if stage == .openConn || stage == .readResponse {
                ...
                var frame : FrameWS?
                if let error = error as? WebSocketErrorController{
                    switch error {
                    case .network(let details):
                        if details == atEndDetails{
                            stage = .closeConn
                            frame = FrameWS.makeClose(1006, reason: "Abnormal Closure")
                            atEnd = true
                            finalError = nil
                        }
                    case .protocolError:
                        frame = FrameWS.makeClose(1002, reason: "Protocol error")
                    case .payloadError:
                        frame = FrameWS.makeClose(1007, reason: "Payload error")
                    default:
                        break
                    }
                }
    ...
    func stepBuffers(_ more: Bool) throws {
        if rd != nil {
            if stage != .closeConn && rd.streamStatus == Stream.Status.atEnd  {
                ...
                throw WebSocketErrorController.network(atEndDetails)
            }
            if more {
                while rd.hasBytesAvailable {
                    ...
                    if size > inputBytesSize {
                        ...
                        if ptr == nil {
                            throw WebSocketErrorController.memory
                        }
    ...
    func stepStreamErrors() throws {
        if finalError == nil {
            if connectionTimeout {
                throw WebSocketErrorController.network(timeoutDetails)
            }
            if let error = rd?.streamError {
                throw WebSocketErrorController.network(error.localizedDescription)
            }
    ...
    var readStateFrame : FrameWS?
    var readStateFinished = false
    var leaderFrame : FrameWS?
    func readFrame() throws -> FrameWS {
        var frame : FrameWS
        var finished : Bool
        if !readStateSaved {
            if leaderFrame != nil {
            ...
            if frame.code == .continue{
                throw WebSocketErrorController.protocolError("leader frame cannot be a continue frame")
            }
            if !finished {
                readStateSaved = true
                readStateFrame = frame
                readStateFinished = finished
                throw WebSocketErrorController.needMoreInput
            }
        } else {
            ...
            if !finished {
                ...
                if cf.code != .continue {
                    if !cf.code.isControl {
                        throw WebSocketErrorController.protocolError("only ping frames can be interlaced with fragments")
                    }
                    leaderFrame = frame
                    return cf
                }
        ...
        if !frame.utf8.completed {
            throw WebSocketErrorController.payloadError("incomplete utf8")
        }
    ...
    func openConn() throws {
        ...
        if req.url == nil || req.url!.host == nil{
            throw WebSocketErrorController.invalidAddress
        }
    ...
    func readResponse() throws {
        ...
        if string == nil {
            throw WebSocketErrorController.invalidHeader
        }
        let header = string!
        ...
        for i in 0 ..< lines.count {
            ...
            if i == 0  {
                if !line.hasPrefix("HTTP/1.1 101"){
                    throw WebSocketErrorController.invalidResponse(line)
                }
            } else if line != "" {
                var value = ""
        ...
        if needsCompression {
            if serverMaxWindowBits < 8 || serverMaxWindowBits > 15 {
                throw WebSocketErrorController.invalidCompressionOptions("server_max_window_bits")
            }
            if serverMaxWindowBits < 8 || serverMaxWindowBits > 15 {
                throw WebSocketErrorController.invalidCompressionOptions("client_max_window_bits")
            }
            inflater = Inflater(windowBits: serverMaxWindowBits)
            if inflater == nil {
                throw WebSocketErrorController.invalidCompressionOptions("inflater init")
            }
            deflater = Deflater(windowBits: clientMaxWindowBits, memLevel: 8)
            if deflater == nil {
                throw WebSocketErrorController.invalidCompressionOptions("deflater init")
            }
        }
    ...
    var fragStatePayload = PayloadAlpha()
    var fragStateStatusCode = UInt16(0)
    var fragStateHeaderLen = 0
    var buffer = [UInt8](repeating: 0, count: windowBufferSize)
    var reusedPayload = PayloadAlpha()
    func readFrameFragment(_ leader : FrameWS?) throws -> FrameWS {
        var inflate : Bool
        var len : Int
        var fin = false
        var code : OpCode
        ...
        if fragStateSaved {
            ...
            if inflater != nil && (rsv1 || (leader != nil && leader!.inflate)) {
                ...
                throw WebSocketErrorController.protocolError("invalid extension")
            } else {
                inflate = false
            }
            code = OpCode.binary
            if let c = OpCode(rawValue: (b & 0xF)){
                ...
                throw WebSocketErrorController.protocolError("invalid opcode")
            }
            if !fin && code.isControl {
                throw WebSocketErrorController.protocolError("unfinished control frame")
            }
            b = try reader.readByte()
            if b >> 7 & 0x1 == 0x1 {
                throw WebSocketErrorController.protocolError("server sent masked frame")
            }
            ...
            if bcount != 0 {
                if code.isControl {
                    throw WebSocketErrorController.protocolError("invalid payload size for control frame")
                }
                len64 = 0
                var i = bcount-1
                while i >= 0 {
            ...
            if code == .continue {
                if code.isControl {
                    throw WebSocketErrorController.protocolError("control frame cannot have the 'continue' opcode")
                }
                if leader == nil {
                    throw WebSocketErrorController.protocolError("continue frame is missing it's leader")
                }
            }
            if code.isControl {
                if leader != nil {
                ...
                if inflate {
                    throw WebSocketErrorController.protocolError("control frame cannot be compressed")
                }
            }
            statusCode = 0
            if leader != nil {
            ...
            if leaderCode == .close {
                if len == 1 {
                    throw WebSocketErrorController.protocolError("invalid payload size for close frame")
                }
                if len >= 2 {
                    ...
                    if statusCode < 1000 || statusCode > 4999  || (statusCode >= 1004 && statusCode <= 1006) || (statusCode >= 1012 && statusCode <= 2999) {
                        throw WebSocketErrorController.protocolError("invalid status code for close frame")
                    }
                }
            }
    ...
    func writeFrame(_ f : FrameWS) throws {
        if !f.finished{
            throw WebSocketErrorController.libraryError("cannot send unfinished frames")
        }
        var hlen = 0
        let b : UInt8 = 0x80
        var deflate = false
...
open class WebSocket: NSObject {
    ...
    open var event : WebSocketEventsAlpha{
        get { return ws.event }
